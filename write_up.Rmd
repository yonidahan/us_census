...
All the scripts needed to perform the following are in my [github repository](https://github.com/yonidahan/us_census).   
...
Also, one can observe some conflicting aspects in the data: some people have a net capital income (dividends+capital gains - capital losses) greater than 50k$ and belong to the low-income class; and many capital_gains of $99,999 seems unreasonable.   
...
In order to handle this unbalanced dataset, a rigorous model would use an oversampling technique such as Synthetic Minority Over-sampling Technique (NV Chawla - 2002), but it's of the scope of this evaluation.   
...
###Feature selection  
Random forests can assess variable importance and out-of-bag error internally, it can be used to obtain a relevant feature subset. A similar but more precise way to obtain all relevant features would be to use the Boruta algorithm.

```{r echo=TRUE,message=FALSE,fig.height=9} 
...
The most important features seem to be **"education"**, **"major_industry_code"**, **"age"**, **"major_industry_code"** (as pointed out in exploratory data analysis), **"dividends_from_stocks"** and **"capital_gains"**.   
...
The classifier using the subset of features performs better, but the recall and F1-score are lower than the ones obtained using random forest.   
...
As noted in the instructions, I spent several hours on the data set (not days). If I were to build a pure model, I would focus on these points:   
...
- try other approaches, like **anomaly detection**   
